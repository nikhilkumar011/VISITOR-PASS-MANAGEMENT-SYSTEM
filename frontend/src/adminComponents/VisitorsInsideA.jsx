...
import VisInsCard from './VisInsCard.jsx'
import PopupBox from '../employeeComponents/PopupBox'


const VisitorsInsideA = () => {
  ...
  return (
    <>
     ...
     <Toast message={popup} />
            ...
            {!loading && visitors.filter(each => each.checkedIn === true).filter((each) => each.approvedStatus === 'Approved').length === 0 && (
                <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
                    <p className="text-3xl md:text-4xl font-semibold tracking-wide text-gray-600">
                        Visitors inside will appear here
                    </p>
            ...
            {!loading && visitors.filter(each => each.checkedIn === true).filter((each) => each.approvedStatus === 'Approved').length !== 0 && (
                ...
                <table className='bg-white w-[95%] border-separate border-spacing-y-4 text-center m-auto my-5 p-6 rounded-2xl shadow-2xl'>
                    ...
                    {
                        visitors.filter((each) => each.checkedIn === true).filter((each) => each.approvedStatus === 'Approved').filter((each) => (each.firstname + each.lastname).toLowerCase().includes(searchBarText.toLowerCase()))
                            .map((each) => (
                                <VisInsCard visitor={each} key={each._id} onDelete={handleDelete} onRowClick={setSelectedVisitor} />
                            ))
                    }
                </table></div>
            )}
...
export default VisitorsInsideA
